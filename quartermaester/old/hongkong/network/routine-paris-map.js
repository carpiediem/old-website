...
function ready(error, v,s,e,n) {
	...
	d3.selectAll(".lignes").append("title").text(function(d) {return (e[d.id].active?"actif":"dÃ©sactivÃ©")})
	...
	legend0.append("text").attr({y:25,x:190}).text("Ã©loignÃ©e")
	legend0.append("text").attr({y:45}).text("ligne").style("font-weight","bold");
	legend0.append("text").attr({y:45,x:60}).text("chargÃ©e")
	legend0.append("text").attr({y:45,x:190}).text("peu frÃ©quentÃ©e")
	legend0.append("text").attr({y:65}).text("tronÃ§on").style("font-weight","bold");
	legend0.append("text").attr({y:65,x:60}).text("actif")
	legend0.append("text").attr({y:65,x:190}).text("dÃ©sactivÃ©")
	

	var legend1=legend.append("g").attr({id:"legend1",transform:"translate(670,0)","text-anchor":"end"}).style("opacity",0);
	legend1.append("text").attr({y:15,x:285}).text("Seuls les chemins les plus courts sont affichÃ©s.")
...
function repos(nodes,edges,stations, votes,s, dist) {
	...
	if(!s) {
		...
		d3.select("em").html("Cliquez sur une station pour voir les temps de trajet, cliquez sur un tronÃ§on pour le dÃ©sactiver")
		stations.forEach(function(d) {
			d.projx=d.ox;
			d.projy=d.oy;
		})
...
function lowerPart(station,nodes,edges,stations,votes) {
	...
	d3.selectAll(".vote").on("mousemove",function(d,i) {
		if(!d.voted) {
			...
			if(i<edgesS.length) {
				p=["On rentre sur le quai !","Un escalier, un couloir...","Station normale","Assez loin de l'entrÃ©e","Voyage au centre de la terre"];
				cont="#slegend";
			} else {
				p=["Les quais sont Ã  cÃ´tÃ© !","TrÃ¨s rapide","Quelques couloirs","On doit pas mal marcher","Un vrai dÃ©dale"];
				cont="#clegend";
			}
...
function distll(s1,s2) {
	var acos=Math.acos,cos=Math.cos,sin=Math.sin,pi=Math.PI;
	var a=s1.lat*pi/180;b=s2.lat*pi/180;c=s1.lon*pi/180;d=s2.lon*pi/180;
